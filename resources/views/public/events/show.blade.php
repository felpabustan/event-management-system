            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center py-6">
                    ...
                    <div class="flex items-center space-x-4">
                        <a href="{{ route('events.public.index') }}" class="text-gray-700 hover:text-gray-900">← Back to Events</a>
                            ...
                            <div class="px-6 py-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500">{{ $event->current_capacity }} / {{ $event->max_capacity }} registered</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            @else
                                                <p class="text-sm text-blue-600 font-semibold">{{ $event->getFormattedPriceAttribute() }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mb-4">
                                    <div class="flex items-center justify-between mb-2">
                                        ...
                                        <span class="text-sm text-gray-500">{{ $event->current_capacity }} / {{ $event->max_capacity }}</span>
                                    </div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">{{ $event->availableSpots() }} spots remaining</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="bg-white overflow-hidden shadow rounded-lg">
                            <div class="px-6 py-6">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    @if($event->isFree())
                                        <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full bg-green-100 text-green-800">Free Event</span>
                                    @else
                                        <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full bg-blue-100 text-blue-800">{{ $event->getFormattedPriceAttribute() }}</span>
                                    @endif
                                </div>
                                ...
                                @if($event->isFull())
                                    <div class="text-center py-8">
                                        ...
                                        <p class="mt-1 text-sm text-gray-500">This event has reached maximum capacity.</p>
                                    </div>
                                        ...
                                        <form action="{{ route('events.register', $event) }}" method="POST">
                                            ...
                                            <div class="space-y-4">
                                                <div>
                                                    ...
                                                    <input type="text" name="name" id="name" value="{{ old('name') }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                                    @error('name')
                                                ...
                                                <div>
                                                    ...
                                                    <input type="email" name="email" id="email" value="{{ old('email') }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                                    @error('email')
                                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                                    @enderror
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    <input type="tel" name="phone" id="phone" value="{{ old('phone') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                                    @error('phone')
                                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                                    @enderror
                                                </div>
                                            ...
                                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                                <div class="flex items-center">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-sm text-blue-600">Price: {{ $event->getFormattedPriceAttribute() }}</p>
                                                    </div>
                                                </div>
                                            </div>

                                            <!-- Registration form for all users -->
                                            <form action="{{ route('payment.checkout', $event) }}" method="POST" id="paymentForm">
                                                @csrf
                                                
                                                <div class="space-y-4 mb-4">
                                                    <div>
                                                        <label for="payment_name" class="block text-sm font-medium text-gray-700">Full Name</label>
                                                        <input type="text" name="name" id="payment_name" value="{{ old('name') }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                                        @error('name')
                                                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                                        @enderror
                                                    </div>
                                                    ...
                                                    <div>
                                                        <label for="payment_email" class="block text-sm font-medium text-gray-700">Email Address</label>
                                                        <input type="email" name="email" id="payment_email" value="{{ old('email') }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                                        @error('email')
                                                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                                        @enderror
                                                    </div>
